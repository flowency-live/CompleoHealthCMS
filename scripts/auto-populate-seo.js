/**
 * Automated script to populate Page SEO data in Strapi via API
 * 
 * Prerequisites:
 * 1. Strapi must be running (npm run develop)
 * 2. Public permissions must be set for page-seo:create and page-seo:update
 * 
 * Run: node scripts/auto-populate-seo.js
 */
...
const seoPages = [
  ...
  },
  {
    ...
    description: "Professional managed equipment services for MRI and CT scanners. Complete lifecycle management including maintenance, upgrades, and technical support.",
  ...
  },
  {
    ...
    description: "Professional clinical insourcing services providing qualified radiographers and technical staff for medical imaging departments.",
  ...
  },
  {
    ...
    description: "Supporting NHS community diagnostic centres with advanced imaging equipment and clinical expertise to reduce waiting times.",
  ...
  },
  {
    ...
    description: "Comprehensive health screening programmes using advanced imaging technology. Early detection and preventive healthcare services.",
  ...
  },
  {
    ...
    description: "Learn how Compleo Health protects your personal data and privacy. Our comprehensive privacy policy explains data collection, usage, and your rights.",
  ...
  },
  {
    ...
    description: "Understand how Compleo Health uses cookies to improve your website experience. Learn about cookie types, management options, and privacy choices.",
  ...
  },
  {
    ...
    description: "Learn about Compleo Health's commitment to digital accessibility and web inclusion. Our accessibility statement details WCAG 2.1 compliance.",
    keywords: "accessibility statement, web accessibility, WCAG compliance, digital inclusion, assistive technology, screen reader support, keyboard navigation"
  },
  {
...
async function populateSEO() {
  try {
    ...
    console.log('Checking for existing Page SEO data...');
    const checkResponse = await axios.get(`${STRAPI_URL}/api/page-seo`);
    
    if (checkResponse.data.data && checkResponse.data.data.id) {
      console.log('Page SEO data already exists. Updating...');
      
      // Update existing entry
      const updateResponse = await axios.put(
        `${STRAPI_URL}/api/page-seo`,
        {
          data: {
            pages: seoPages
          }
      ...
      console.log('✅ Page SEO data updated successfully!');
      console.log(`Updated ${seoPages.length} page SEO entries`);
    } else {
      console.log('No existing Page SEO data found. Creating new...');
      ...
      console.log('✅ Page SEO data created successfully!');
      console.log(`Created ${seoPages.length} page SEO entries`);
    }
    
    console.log('\n📝 Note: Remember to:');
    console.log('1. Set public permissions for page-seo:find in Strapi Admin');
    console.log('2. Publish the content in Strapi Admin if using draft/publish');
    
  } catch (error) {
    if (error.response?.status === 403) {
      console.error('❌ Permission denied. Please configure permissions in Strapi Admin:');
      console.error('1. Go to Settings → Users & Permissions Plugin → Roles → Public');
      console.error('2. Enable create/update permissions for Page-seo');
      console.error('3. Save and try again');
    } else if (error.response?.status === 404) {
      console.error('❌ Page SEO endpoint not found. Make sure:');
      console.error('1. Strapi is running (npm run develop)');
      console.error('2. The page-seo content type has been created');
    } else {
      console.error('❌ Error populating SEO data:', error.message);
